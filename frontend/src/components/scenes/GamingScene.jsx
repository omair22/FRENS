...
const getPositions = (count, sceneWidth, sceneHeight) => {
  const yOffset = sceneHeight * 0.05
  const seatY = sceneHeight - 5 + yOffset
  const standY = sceneHeight - 5 + yOffset
  const pos = []

  // Sitting shifted a bit to the left
  // Standing pulled in so not cut off on right
...
export const startIdleAnimation = (data) => {
  ...
  const isStanding = sprite.pose === 'standing'

  gsap.killTweensOf(sprite)

  if (rsvp.response === 'in') {
    gsap.to(sprite, { rotation: 0.02, duration: 3, ease: 'sine.inOut', yoyo: true, repeat: -1, delay: offset })
    gsap.to(sprite, { y: baseY + (isStanding ? 1 : 2), duration: 2, ease: 'sine.inOut', yoyo: true, repeat: -1, delay: offset })

    const armGroup = sprite.getChildByName('armGroup')
    if (armGroup) {
      const playTL = gsap.timeline({ repeat: -1, delay: 2 + (index * 1.5) })
      playTL
        .to(armGroup, { rotation: 0.2, duration: 0.3, ease: 'power2.inOut', yoyo: true, repeat: 5 })
        .to({}, { duration: 4 + (index * 2) })
    }
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      await Promise.all(displayRsvps.map(async (rsvp, i) => {
        ...
        const container = await buildFullAvatar(rsvp.user || { name: 'user' }, 'gaming', pos.pose)
        container.x = pos.x
        container.y = pos.y
        container.pose = pos.pose
        // Slightly smaller scale to avoid huge overlap and headless clipping
    ...
    <div style={{ width: '100%' }}>
      <div ref={wrapperRef} style={{ position: 'relative', width: '100%', height, background: '#1a1a1a', borderRadius: overflowRsvps.length > 0 ? '12px 12px 0 0' : '12px', overflow: 'hidden' }}>
        <div style={{
          ...
          backgroundImage: 'url(/scenes/gaming/bg.png)',
